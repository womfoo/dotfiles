...
with lib;
{
  ...
  config =
    let
      ...
      getPeer = host : { inherit (constants.devices."${host}".interfaces.wg0) publicKey allowedIPs endpoint; };
                  ...
                  in
                    {
                      ...
                      allowedIPs = if cfg.makeServer then [ "${iface.ip}/32" ] else [ "10.0.0.0/8" ] /* [ "0.0.0.0/0" ] */ ;
